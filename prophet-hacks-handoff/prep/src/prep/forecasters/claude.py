...
def _resolve_agent_prompt(config: ForecasterConfig) -> str:
    if getattr(config, "agent_prompt", None):
...
def forecast(config: ForecasterConfig, packet: MarketPacket) -> ModelForecast:
    ...
    if market_implied and not packet.is_binary:
        # Per-outcome Kalshi prices take precedence over the shell binary mid for
        # multi-outcome events — give Claude the data it needs to anchor correctly.
        market_block["market_implied_probabilities"] = market_implied
    market_data = packet.retrieval.get("market_data")
    if market_data and not packet.is_binary:
        market_block["market_data"] = market_data
